...
The `uic.loadUi` function of PyQt5 as well as the `QtUiTools.QUiLoader().load` function of PySide2 are mapped to a convenience function in Qt.py called `loadUi` (part of the `QtCompat` module; `QtCompat.loadUi`). This function only accepts the filename argument of the .ui file.
